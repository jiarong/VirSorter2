...
rule hmmsearch:
    ...
    shell:
        ...
        hmmsearch -T {Hmmsearch_score_min} --tblout {output} --cpu {threads} --noali -o /dev/null $Hmmdb $Inputseq 2> {log} || {{ echo "See error details in {log}" | python {Scriptdir}/echo.py --level error; exit 1; }}

        if [ "$To_scratch" = true ]; then
